return {
  front(shard){
  let page=`<!DOCTYPE html><html lang=en style="--color:${manifest.color||"#e91e63"};--background:${manifest.background||"#fff"};--shadow:${manifest.shadow||"#00000020"};--text:${manifest["text color"]||"#454545"}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="${manifest.color||"#e91e63"}"><link rel="apple-touch-icon" href="${manifest.icon||"/favicon.ico"}"><link rel="manifest" href="data:application/json;base64,${btoa(JSON.stringify(webmanifest))}">${shard.scriptSrc.map(e=>`<script src="${e}"></script>`).join("")}${shard.styleSrc.map(e=>`<link rel="stylesheet" href="${e}">`).join("")}<style>${shard.style.join("\n")}</style>`
